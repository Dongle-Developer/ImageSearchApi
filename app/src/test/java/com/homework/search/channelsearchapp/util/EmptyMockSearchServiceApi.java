...
public class EmptyMockSearchServiceApi implements SearchApiService {

   private final BehaviorDelegate<SearchApiService> delegate;

   public EmptyMockSearchServiceApi(BehaviorDelegate<SearchApiService> delegate) {
   ...
   public Observable<SearchResponse> getSearchDataImage(String query, int page, String sort,
      ...
      try {
         testJson = JsonUtil.getStringFromFile("test_json_empty.json");
      } catch (Exception e) {
         e.printStackTrace();
      }
      Gson gson = new Gson();
